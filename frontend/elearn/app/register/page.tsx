...
const RegisterPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (avatar) {
      ...
      console.log("Avatar appended:", avatar);
    } else {
        ...
        <div className="max-w-md w-full p-6">
          ...
          <form method="POST" className="space-y-4" onSubmit={handleSubmit} encType="multipart/form-data">
            ...
            <div>
              ...
              <input type="password" id="password" name="password" onChange={(e) => setPassword(e.target.value)} className="mt-1 p-2 w-full border rounded-md" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray">تأكيد كلمة المرور</label>
              <input type="password" id="password2" name="password2" onChange={(e) => setPassword2(e.target.value)} className="mt-1 p-2 w-full border rounded-md" />
            </div>
